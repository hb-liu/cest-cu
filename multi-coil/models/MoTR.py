...
class Transformer(nn.Module):
    def __init__(self, idim, hdim, odim, dim_head=16, heads=8, num_blocks=2):
        super().__init__()
...
class MoTR(nn.Module):
    def __init__(self, modelparams):
        ...
        for i in range(niter):
            self.iters.append(Transformer(2*nadj, hdim, 2*nadj))
        self.rate = rate
        self.nadj = nadj
    @torch.no_grad()
    def undersampling(self, kf, mode) -> Tensor:
        """
        perform retrospective undersampling of fullysampled image
        Args:
            kf: fullysampled k-space, `(batchsize, ncoils, nadj, height, width)`
            mode: 'coilwise' or 'sense'
        Returns:
            ku: undersampled k-space, `(batchsize, ncoils, nadj, height, width)`
        """
        if mode == 'sense':
            # number of center/peripheral kspace to be sampled
            height = kf.shape[-2]
            nce = npe = height//self.rate//2
            # index of center/peripheral kspace
            c = list(range((height-nce)//2, (height+nce)//2))
            p = np.setdiff1d(list(range(height)), c)
            # sampling
            np.random.shuffle(p)
            ...
            for i in range(self.nadj):
                ...
                ku[:,i,p[i]] = kf[:,i,p[i]]
            return ku
    def forward(self, ku, csm=None, mode='sense') -> Tensor:
        """
        Args:
            ku:   undersampled k-space, `(batchsize, ncoils, nadj, height, width)`
            csm:  coil sensitivity maps, `(batchsize, ncoils, 1, height, width)`
            mode: 'coilwise' or 'sense'
        Returns:
        ...
        if mode == 'sense':
            x = ifft2c(ku)
            batchsize, ncoils = x.shape[0], x.shape[1]
            x  = rearrange(x, 'b c n h w -> (b c) n h w')
            ku = rearrange(ku, 'b c n h w -> (b c) n h w')
            for layer in self.iters:
                x = x + layer(x)
                # data consistency
                kdc = fft2c(x) * (ku == 0) + ku
                x = ifft2c(kdc)
            x = rearrange(x, '(b c) n h w -> b c n h w', b = batchsize)
            cb = sense(x, csm)
            return x, cb
        elif mode == 'coilwise':
            x = ifft2c(ku)
